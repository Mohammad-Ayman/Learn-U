...
const Navbar = () => {
  ...
  if (["/", "/signin", "/statistics"].includes(pathname)) {
    return null;
  }
        ...
        {navItems.map((item) => (
          <Link href={item.href} key={item.href}>
            <div
              className={
                pathname.startsWith(item.href)
                  ? "text-blue-400 hover:text-blue-400 cursor-pointer"
            ...
            >
              ...
              <p className="text-l font-bold">{item.label}</p>
            </div>
          </Link>
        ))}
        <Link href="/signin">
          <div
            onClick={logOut}
            className="text-gray-400 hover:text-blue-400 cursor-pointer mb-4"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
